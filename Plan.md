...
3.10. **[原生 Gemini 源 · ✅ 已加(用户 2026-06-16,对象在西班牙能直连 Google)]** providers.ts 加 `{id:'google',kind:'pi',piProvider:'google',models:[gemini-2.5-flash/2.5-pro/3-flash-preview]}`。实测 pi.getModel('google',…) 可取、api=google-generative-ai、reasoning=true、vision。key 填 Gemini API Key(pi-ai 走 config.apiKey)。**无需反代**;反代预设保留仅供 Google 受限地区。typecheck+254测过。
   ...
   - **「只填 key」水平已达成(用户 2026-06-18 要求)**:原生 pi 源(google/minimax/deepseek/zai/moonshotai)不显示接口地址栏、只填 key;openai 兼容源(通义)接口地址已自动预填官方值。她在设置选源+粘 key+保存即可,无其他必填项。
   - **国内/国外厂商可达性(用户 2026-06-18 问;接口可达≠能注册到 key 两层)**:西班牙推荐**国际版**——Google Gemini 原生/DeepSeek/MiniMax 国际/智谱国际/Kimi 国际(国际可达、邮箱注册)。国内版(minimax-cn/zai-coding-cn/moonshotai-cn)端点也许可达但**注册多需国内手机号**、不保证;通义默认填国内 dashscope 地址、国外建议换国际版。坑在注册非墙。**实际西班牙可达/延迟仅当地真机可验**(见交付清单真机项)。
   - **❌ 已否决:接 Antigravity/Gemini CLI OAuth 免费模型(用户 2026-06-18 问+Web 查证)**:市面确有开源代理(CLIProxyAPI/AIClient2API/frieser-antigravity-proxy/opencode-antigravity-auth)把 Gemini CLI/Antigravity 的账号 OAuth 包成 OpenAI 兼容接口,小狗经反代源技术上能接。**但强烈否决**:① Google 2026-02 已禁此 token 代理、3-25 起检测执行、**大批账号被封含付费 Ultra/Pro**→用对象 Google 账号走代理=可能害其主账号(连带 Pro 会员)被封;② **就在 2026-06-18 Gemini CLI 停服 Pro/Ultra/免费层转 Antigravity CLI**,这些代理正动荡失效;③ 违 ToS+需额外本地代理+她做 OAuth=更复杂。**定调:坚持免费 AI Studio key 走原生 google 源,同样免费 Gemini、官方稳定不封号。**
...
- [x] ~~把 GitHub 仓库设为 public + 推代码~~ ✅ 2026-06-16:已设 PUBLIC + push(8f4fd37),匿名 clone 验证通过。
- [x] ~~删临时诊断代码(diag/3.9)~~ ✅ 2026-06-18 上线前已整段移除(showInactive 跳桌面修复保留)。
- [x] ~~补 LICENSE~~ ✅ 2026-06-18 加 LICENSE(MIT)。
- [x] ~~推 GitHub~~ ✅ 已与远端同步(bd9b741)。
- [ ] **跳桌面(3.7/3.9)真机双桌面终验**——已"暂时修复"(showInactive),探头已撤;若她那边复现再重新埋点定位。
- [ ] README 占位截图换真图(可上线后补,非阻塞)。

### ✅ 2026-06-18 上线前总检查结论(/goal)
- **测试全绿**:typecheck + 260 离线测 + build + E2E 23/23。code-reviewer 审本会话改动出 3 bug(0C/0H)全修。
- **真机模拟部署通过**:全新匿名 clone 公开仓库→npm install(250 包)→build(产物齐)→typecheck 全过 = 对象能从公开仓库一路装起。
- **仅剩真机项**(我替不了,需她西班牙那台 Mac):① 跳桌面终验 ② 当地网络实测某国际源可达 ③ 干净机 TCC 授权(提醒/日历/通知)。
- **已知非本程问题**:MiniMax-M3 服务端偶发 500(已优雅兜底成友好提示)、工具触发~10% 非确定性(铁律已缓解)。
