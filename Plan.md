...
**可行替代**：① **on-demand(现在就能用)**——她直接问「帮我找马德里租房房源」→ web_search 搜(能搜到小红书帖+idealista+论坛)。② **可选·每天自动 digest**——像晨报一样定时 web_search「马德里 租房」给她几条新链接(需设计投递:proactive 现在只走通知+气泡，房源链接更适合落进当前会话或专门面板；去重)。建议先用 on-demand，自动 digest 待用户要再做。
